...
volatile u16 plen = 0;
volatile u08 html_check[16] = {0x47,0x45,0x54,0x20,0x2f,0x20,0x48,0x54,0x54,0x50,0x2f,0x31,0x2e,0x31,0x0d,0x0a};
...
extern bool net_analysis(void){
	...
	if (plen == 0){ 
		...
		if (protocol == TCP_IP){
			if (com_arr((u08*)&rx_buf[I_TCP_DATA], (u08*)html_check, 16)){
				protocol = HTML;
			}
		}
...
extern void net_tcp_ip_execute(u08* data, u16 len){
	...
	else if (com_arr((u08*)&rx_buf[I_TCP_DATA], (u08*)html_check, 16)){
		net_tcp_ip_request(data, len, (u08*)"HTTP/1.1 200 OK\r\nContent-Type: text/html\r\n\r\n <h1>IoT47.com xin chao cac ban</h1>", 81);
		return;
	}
	else{
